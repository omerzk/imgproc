...
%4.: Produce the panorama frames since my pyramid functions only except
%powers of 2
panoSize2 = [0 0 3];
panoSize2(1) = 2^(nextpow2(panoSize(1)));
panoSize2(2) = 2^(nextpow2(panoSize(2)));
frames = zeros([panoSize nViews]);
OkFrames = 1;

for k = 1:nViews
[panoramaFrame,frameNotOK] = renderPanoramicFrame(panoSize2, images,panT, ones(1, size(images,4))*sliceCenters(k),halfSliceWidth );

%5.:MovieMaker
if ~frameNotOK
    %Save the relavent region. 
    frames(: ,:, :, OkFrames) = panoramaFrame(1:panoSize(1),1:panoSize(2),:);
    OkFrames = OkFrames + 1;
end
